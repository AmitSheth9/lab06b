...
app.post('/cafedrinks', async(req, res)=> {
  try{
    ...
    const data = await client.query(`INSERT INTO cafedrinks (name, price, calories, category_id, image, owner_id)
    VALUES($1, $2, $3, $4, $5, $6)
    RETURNING *
    `, [req.body.name, req.body.price, req.body.calories, req.body.category_id, req.body.image, 1]);
    res.json(data.rows[0]);
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
app.put('/cafedrinks/:id', async(req, res)=> {
  try{
    ...
    SET name=$1, price=$2, calories=$3, category_id=$4, image=$5
    WHERE id=$6 
    RETURNING *`, [req.body.name, req.body.price, req.body.calories, req.body.category_id, req.body.image, req.params.id]);
    res.json(data.rows[0]);
  }catch(e) {
    res.status(500).json({ error: e.message });
  }
